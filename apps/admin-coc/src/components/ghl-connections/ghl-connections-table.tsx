...
export function GhlConnectionsTable({
  ...
  return (
    <div className="space-y-4">
      ...
      <Button type="button" onClick={connectGhl} disabled={pending}>
        {pending ? "Starting…" : "Connect GHL subaccount"}
      </Button>
            ...
            {items.length === 0 ? (
              <tr>
                <td colSpan={6} className="px-3 py-6 text-center text-muted-foreground">
                  No GHL connections yet. Use Connect GHL to authorize a subaccount.
                </td>
              </tr>
            ) : (
              items.map((row) => (
                <tr key={row.id} className="border-b border-border/60 align-top">
                  ...
                  <td className="px-3 py-2">
                    <Badge variant="outline" className={cn("w-fit", ghlConnectionStatusBadgeClass(row.connectionStatus))}>
                  ...
                  <td className="px-3 py-2 space-y-2">
                    <div className="flex flex-wrap gap-1">
                      <Button type="button" size="sm" variant="secondary" disabled={pending} onClick={() => probe(row.id)}>
                        Probe
                      </Button>
                      <Button type="button" size="sm" variant="outline" disabled={pending} onClick={() => disconnect(row.id)}>
                        Disconnect
                      </Button>
                    </div>
                    <div className="flex gap-1">
                      ...
                      />
                      <Button type="button" size="sm" disabled={pending} onClick={() => linkClient(row.id)}>
                        Link
                      </Button>
                    </div>
                  </td>
