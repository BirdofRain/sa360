...
import type { ArchitectureFlow, ArchitectureTier } from "../architecture/architecture-types";

/**
 * Historical snapshot recovered from git commit 59afb31 (pre LF pivot).
 * Archived/read-only for comparison only.
 */
export const PRE_PIVOT_ARCHITECTURE_TIERS: ArchitectureTier[] = [
  {
    id: "source",
  ...
  },
  {
    ...
    blocks: [
      ...
      },
      {
        ...
        name: "Worker",
        caption: "BullMQ",
        description: "Background jobs: Meta CAPI dispatch, retry queues, lifecycle re-processing.",
        icon: Cog,
        status: "LIVE",
      },
      {
    ...
    blocks: [
      {
        ...
        description:
          "LifecycleEvent, LeadAttribution, InboundContactIndex, WebhookRequestLog, SynthflowRequestLog, and planned review/flag records.",
        icon: Database,
        status: "LIVE",
      },
      {
        ...
        description: "BullMQ queues, rate-limit counters, and future flag/lookup cache.",
        icon: Zap,
        status: "LIVE",
      },
    ],
...
export const PRE_PIVOT_ARCHITECTURE_FLOWS: ArchitectureFlow[] = [
  {
    id: "flow-lifecycle",
