import type { WorkflowModule } from "../workflow/workflow-types";

export const PRE_PIVOT_WORKFLOW_STATUSES = ["LIVE", "BUILDING", "NEXT", "FUTURE"] as const;

/**
 * Historical snapshot recovered from git commit 59afb31 (pre LF pivot).
 * Archived/read-only for comparison only.
 */
export const PRE_PIVOT_WORKFLOW_MODULES: WorkflowModule[] = [
  {
    id: "M1",
    short: "M1",
    ...
    cards: [
      ...
      },
      {
        ...
        bullets: [
          ...
          "Create review item if failed, unknown, or needs human attention",
          "Emit lifecycle signals (appointment_set, appointment_confirmed, no_answer, failed_call)",
        ],
      },
    ],
    branches: [
