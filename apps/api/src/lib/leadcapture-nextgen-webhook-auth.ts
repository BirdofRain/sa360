...
export type LeadCaptureNextGenWebhookAuthResult =
  | { ok: true; devWarning?: string; method: "header" | "basic" }
  | { ok: false; reason: "missing" | "invalid" | "integration_not_configured"; hint?: string };

const DEFAULT_BASIC_AUTH_USERNAME = "sa360-leadcapture-nextgen";
const HEADER_NAME = "x-sa360-leadcapture-nextgen-key";
...
function readEnvSecret(): string {
  const envRaw = process.env.SA360_LEADCAPTURE_NEXTGEN_WEBHOOK_SECRET;
  return typeof envRaw === "string" ? envRaw.trim() : "";
}

function readBasicAuthUsername(): string {
  const envRaw = process.env.SA360_LEADCAPTURE_NEXTGEN_BASIC_AUTH_USERNAME;
...
/**
 * Validates LeadCapture Next-Gen webhook auth via:
 * - `x-sa360-leadcapture-nextgen-key` header, or
 * - HTTP Basic Auth (username from env, password = Next-Gen webhook secret).
 *
 * Uses SA360_LEADCAPTURE_NEXTGEN_WEBHOOK_SECRET (dedicated from legacy).
 * Fail-closed in production when secret unset; permissive in dev with warning.
 */
export function validateLeadCaptureNextGenWebhookAuth(input: {
  headerKey?: string;
  authorizationHeader?: string;
}): LeadCaptureNextGenWebhookAuthResult {
  ...
  if (!env) {
    if (isProductionEnvironment()) {
      return {
        ...
        hint: "Set SA360_LEADCAPTURE_NEXTGEN_WEBHOOK_SECRET in the API environment.",
      };
    }
    return {
      ok: true,
      method: "header",
      devWarning:
        "SA360_LEADCAPTURE_NEXTGEN_WEBHOOK_SECRET is not set — webhook accepted without key validation (dev only).",
    };
  }
...
export const LEADCAPTURE_NEXTGEN_AUTH_HEADER = HEADER_NAME;
