...
function readEnvSecret(): string {
  const envRaw = process.env.SA360_LEADCONDUIT_WEBHOOK_SECRET;
...
/**
 * Validates LeadConduit webhook auth via:
 * - `x-sa360-leadconduit-key` header, or
 ...
 */
export function validateLeadConduitWebhookAuth(input: {
  headerKey?: string;
  authorizationHeader?: string;
}): LeadConduitWebhookAuthResult {
  ...
  if (!env) {
    ...
    return {
      ...
      method: "header",
      devWarning:
        "SA360_LEADCONDUIT_WEBHOOK_SECRET is not set — webhook accepted without key validation (dev only).",
    };
  }
  ...
  if (!incomingHeader) return { ok: false, reason: "missing" };
  return { ok: false, reason: "invalid" };
}
