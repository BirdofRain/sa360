...
test("GET /admin/v1/routing/dry-run-decisions → lists all masters when masterClientAccountId omitted", async () => {
  const prev = process.env.ADMIN_API_KEY;
  process.env.ADMIN_API_KEY = "admin-secret";
  const app = await buildApp();
  const res = await app.inject({
    method: "GET",
    url: "/admin/v1/routing/dry-run-decisions?limit=5",
...
test("GET /admin/v1/routing/dry-run-master-clients → 401 without admin key", async () => {
  const prev = process.env.ADMIN_API_KEY;
  process.env.ADMIN_API_KEY = "admin-secret";
  const app = await buildApp();
  const res = await app.inject({
    method: "GET",
    url: "/admin/v1/routing/dry-run-master-clients",
  });
  assert.equal(res.statusCode, 401);
  await app.close();
  if (prev !== undefined) process.env.ADMIN_API_KEY = prev;
...
test("PATCH /admin/v1/routing/dry-run-decisions/:id/validation → 401 without admin key", async () => {
  const prev = process.env.ADMIN_API_KEY;
  process.env.ADMIN_API_KEY = "admin-secret";
  const app = await buildApp();
  const res = await app.inject({
    method: "PATCH",
    url: "/admin/v1/routing/dry-run-decisions/dec_1/validation",
    payload: { validationStatus: "matched_legacy" },
  });
  assert.equal(res.statusCode, 401);
  await app.close();
  if (prev !== undefined) process.env.ADMIN_API_KEY = prev;
...
test("PATCH /admin/v1/routing/dry-run-decisions/:id/validation → 400 on invalid status", async () => {
  ...
  const res = await app.inject({
    ...
    payload: { validationStatus: "not_a_status" },
  });
  assert.equal(res.statusCode, 400);
  await app.close();
  if (prev !== undefined) process.env.ADMIN_API_KEY = prev;
