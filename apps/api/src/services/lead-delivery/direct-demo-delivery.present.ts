...
export function inferDirectDemoSourceLane(payload: unknown): {
  ...
  const attribution = extractRoutingAttributionFromPayload(payload as LifecycleEventSchema);
  const platform = normalizeToken(attribution.sourcePlatform);
  const sourceType = normalizeToken(attribution.sourceType);
