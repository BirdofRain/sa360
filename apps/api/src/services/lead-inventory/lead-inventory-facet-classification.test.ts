...
function classifyFromSqlStyle(input: {
  ...
  category: "available" | "reserved" | "blocked";
  cell: { total: number; available: number; reserved: number; blocked: number; supply: number };
} {
...
test("classification regression: representative matrix preserves partition invariant", () => {
  ...
  const scenarios = [
    ...
    {
      sourceLane: "facebook_meta_lead_ads",
      proofStatus: "UNREVIEWED",
      verificationStatus: "FAILED",
      duplicateStatus: "UNIQUE",
      hasHold: false,
  ...
  assert.equal(
    assertFacetCellInvariants({ total, available, reserved, blocked, supply: available + reserved }),
    true
  );
});
